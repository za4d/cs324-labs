        ...
        for ( var ParticleK = 0; ParticleK < numParticles; ParticleK++ ) { /* repulsive force */
            if ( ParticleK != ParticleI ){
               var t = 
                ...
                var d2 = dot(t, t); //Since the projection of a vector on to itself leaves its magnitude unchanged, 
                                    //the dot product of any vector with itself is the square of that vector's magnitude. 
...
var update = function(){
    ...
    // we have to transfer all the data to the GPU at every step - however this is necessary - and cannot be done in the vertex shader - if we consider  
    // interparticle forces, because the vertex-shader works vertex-by-vertex and each particle is a vertex so we would have no knowledge of the other particles.
}
