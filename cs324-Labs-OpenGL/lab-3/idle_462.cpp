...
float g_xoffset_step = 10.0f; // world coord units
float g_yoffset_step = 15.0f; // world coord units
...
void idle()
{
	...
	if ((g_xoffset + g_xoffset_step*2)<=-400){
	  g_xoffset_step*=-1;
	}else if((g_xoffset + g_xoffset_step*2)>=400){
	  g_xoffset_step*=-1;
	}else{
	  g_xoffset += g_xoffset_step;
	}
	if ((g_yoffset + g_yoffset_step*2)<=-400){
	  g_yoffset_step*=-1;
	}else if((g_yoffset + g_yoffset_step*2)>=400){
	  g_yoffset_step*=-1;
	}else{
	  g_yoffset += g_yoffset_step;
	}
...
void special(int key, int, int)
{
	...
	switch (key)
	{
		case GLUT_KEY_LEFT: g_xoffset -= g_xoffset_step; break;
		case GLUT_KEY_RIGHT: g_xoffset += g_xoffset_step; break;
		case GLUT_KEY_UP: g_yoffset += g_yoffset_step; break;
		case GLUT_KEY_DOWN: g_yoffset -= g_yoffset_step; break;
	}
