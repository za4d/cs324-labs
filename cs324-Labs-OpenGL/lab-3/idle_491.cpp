...
unsigned int g_the_rectangle = 0;
...
void display()
{
	...
	glPushMatrix();
		glTranslatef(400.0f+g_xoffset, 400.0f+g_yoffset, 0.0f);
	...
	glPushMatrix();
		glTranslatef(600.0f+g_xoffset, 600.0f+g_yoffset, 0.0f);
		glScalef(100.0f, 100.0f, 1.0f);
		glRotatef(g_angle, 0.0f, 0.0f, 1.0f); 
		
		// execute a pre-compiled display list
		glCallList(g_the_rectangle);
...
void init()
{
	...
	g_the_rectangle = make_rectangle();
